...
void SubscribeInvokerActor::exportInsideStateVariablesTagInXML(ostream &out, string tab){
		...
		BaseActor::exportInsideStateVariablesTagInXML(out, tab);
			    	...
			    	out << tab << '\t' << "<variable name=\"SubscribeInvoker.lastPub\" type=\"int\">" <<  
			    		((int)_ref_lastPub) << "</variable>" << endl;
			    	out << tab << '\t' << "<variable name=\"SubscribeInvoker.monitorId\" type=\"int\">" <<  
			    		((int)_ref_monitorId) << "</variable>" << endl;
			    	out << tab << '\t' << "<variable name=\"SubscribeInvoker.publisherId\" type=\"int\">" <<  
			    		((int)_ref_publisherId) << "</variable>" << endl;
}
...
void SubscribeInvokerActor::marshalVariablesToArray(byte * array){
	int marshalIndex = 0;
                	...
                	memcpy(&array [marshalIndex], &_ref_monitorId, (1 * INT_SIZE));
	                marshalIndex += (1 * INT_SIZE);
            
                	memcpy(&array [marshalIndex], &_ref_publisherId, (1 * INT_SIZE));
	                marshalIndex += (1 * INT_SIZE);
...
void SubscribeInvokerActor::unmarshalVariablesFromArray(byte * array){
	...
	int index = 0;
                	...
                	memcpy(&_ref_monitorId, &array [index], (1 * INT_SIZE));
	                index += (1 * INT_SIZE);

            
            
                	memcpy(&_ref_publisherId, &array [index], (1 * INT_SIZE));
	                index += (1 * INT_SIZE);
...
++i;                    return msgsrvdrop(
    ...
    long SubscribeInvokerActor::constructorSubscribeInvoker (byte creatorId 
...

		{
			(_ref_lastPub=0);
			(_ref_id=_ref__id);
			(_ref_monitorId=_ref__monitorId);
		}
		shift = 0;
		queueTail = 0;
...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void SubscribeInvokerActor::setNow(TIME_TYPE now) {
	    ...
	    long SubscribeInvokerActor::msgsrvpublish (
    		...
    		int
	         _ref_life
    			...
    			long arrayIndexChecker = 0;SubscribeInvokerActor *temp0;MonitoringAppActor *temp1;MonitoringAppActor *temp2;MonitoringAppActor *temp3;
		shift = 1;
		#ifdef SAFE_MODE
			string reactiveClassName = this->getName();
...

		{
			...
			(_ref_Lm=(_ref_Lm+_ref_SI1Delay));
			MonitoringAppActor* _ref_ma = (dynamic_cast<MonitoringAppActor*>(_synchmethod_find(_ref_monitorId)));

			if (((_ref_time-_ref_lastPub)<_ref_NSUB)) {
				{
					(temp0=_ref_self, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 284"), temp0)->_timed_msg_drop(myID, _ref_data, _ref_topic, _ref_Lm, _ref_now, MAX_TIME);
				}
}
			else {
				if (((_ref_time-_ref_lastPub)>_ref_XSUB)) {
					{
						(temp1=_ref_ma, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 287"), temp1)->_timed_msg_slowPublication(myID, _ref_data, _ref_topic, _ref_Lm, _ref_now, MAX_TIME);
					}
}
				else {
					if (((_ref_life<_ref_RSUB)||(_ref_Lm>_ref_RPUB))) {
						{
							(temp2=_ref_ma, assertion(temp2!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 290"), temp2)->_timed_msg_staleDataPub(myID, _ref_data, _ref_topic, _ref_Lm, _ref_now, MAX_TIME);
						}
}
					else {
						{
							(_ref_lastPub=_ref_now);
							(temp3=_ref_ma, assertion(temp3!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 295"), temp3)->_timed_msg_publish(myID, _ref_data, _ref_topic, _ref_Lm, _ref_SI1Delay + _ref_now, MAX_TIME);
						}
}}}
		}
		return 0;
...

		{
			...
			int _ref_unConsumed = 0;

			(_ref_SI2Delay=3);
			MonitoringAppActor* _ref_ma = (dynamic_cast<MonitoringAppActor*>(_synchmethod_find(_ref_monitorId)));

			if (((_ref_time-_ref_lastPub)>_ref_LSUB)) {
				{
					(_ref_unConsumed+=1);
					if ((_ref_unConsumed>_ref_consumptionTolerance)) {
						{
							(temp0=_ref_ma, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 311"), temp0)->_timed_msg_slowConsumption(myID, _ref_data, _ref_topic, (_ref_Lm+_ref_SI2Delay), _ref_now, MAX_TIME);
							(_ref_unConsumed=0);
						}
}
				}
		...
		{
			(temp0=_ref_self, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 316"), temp0)->_timed_msg_busy(myID, _ref_now, MAX_TIME);
		}
		return 0;
