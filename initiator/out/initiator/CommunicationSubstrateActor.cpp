...
#define _ref_self (dynamic_cast<CommunicationSubstrateActor*>(rebecs[myID]))
    	...
    	#define _ref_EI (dynamic_cast<ExecuteInvokerActor*>(rebecs[known[2]]))
...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void CommunicationSubstrateActor::setNow(TIME_TYPE now) {
...

		{
			...
			(temp0=_ref_EI, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 102"), temp0)->_timed_msg_initiate(myID, (_ref_Lm+_ref_cs1Delay), _ref_cs1Delay + _ref_now, MAX_TIME);
		}
		if (__tNumber & 1) {
...

		{
			...
			(temp0=_ref_IR, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 106"), temp0)->_timed_msg_ack(myID, (_ref_Lm+_ref_cs2Delay), _ref_cs2Delay + _ref_now, MAX_TIME);
		}
		if (__tNumber & 1) {
			if (false){/*Dummy 'if' statement to create nested else-if easily!*/}
			else if(nonDetVariable0 == 2) {
		...
		{
			(temp0=_ref_IR, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 109"), temp0)->_timed_msg_excessLoadFailure(myID, _ref_now, MAX_TIME);
		}
		return 0;
		...
		{
			(temp0=_ref_IR, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 112"), temp0)->_timed_msg_timeOutFailure(myID, _ref_now, MAX_TIME);
		}
		return 0;
