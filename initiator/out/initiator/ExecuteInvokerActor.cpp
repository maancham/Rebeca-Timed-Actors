	...
	void ExecuteInvokerActor::setNow(TIME_TYPE now) {
...

		{
			...
			if (((_ref_passedTime<_ref_N_EXE)&(!_ref_theFirsttime))) {
				{
					(temp0=_ref_cs, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 142"), temp0)->_timed_msg_excessLoadFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				{
					startTimer(_ref_lastIni);
					(temp1=_ref_s, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 147"), temp1)->_timed_msg_initiate(myID, _ref_Lm, _ref_EI1Delay + _ref_now, MAX_TIME);
				}
}
...

		{
			...
			if ((_ref_passedTime>_ref_L_EXE)) {
				{
					(temp0=_ref_cs, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 159"), temp0)->_timed_msg_timeOutFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				{
					(temp1=_ref_cs, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 162"), temp1)->_timed_msg_ack(myID, (_ref_Lm+_ref_EI2Delay), _ref_EI2Delay + _ref_now, MAX_TIME);
				}
}
		}
		if (__tNumber & 1) {
