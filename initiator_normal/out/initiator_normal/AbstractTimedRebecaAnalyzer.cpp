#include "AbstractTimedRebecaAnalyzer.h"
		...
		#define _ref_ir (dynamic_cast<InitiateRequesterActor*>(rebecs[1]))
		#define _ref_cs (dynamic_cast<CommunicationSubstrateActor*>(rebecs[2]))
		#define _ref_ei (dynamic_cast<ExecuteInvokerActor*>(rebecs[3]))
...
void AbstractTimedRebecaAnalyzer::instantiationPart() {
	...
	byte maxRebecID = 0;
    	...
    	InitiateRequesterActor* _ref_ir = new InitiateRequesterActor(maxRebecID++, "ir", 20, 4, *referenceToRebecs 
    											#ifdef DEBUG_LEVEL_2 
    											, analysisOutputStream 
    											#endif
    											, 125
        ...
        rebecs.push_back(_ref_ir);
        referenceToRebecs->push_back(_ref_ir);
    	...
    	ExecuteInvokerActor* _ref_ei = new ExecuteInvokerActor(maxRebecID++, "ei", 20, 4, *referenceToRebecs 
    											#ifdef DEBUG_LEVEL_2 
    											, analysisOutputStream 
    											#endif
    											, 125
        ...
        rebecs.push_back(_ref_ei);
        referenceToRebecs->push_back(_ref_ei);
 ...
 _ref_c->known[1] = _ref_ir->myID;

		_ref_c->constructorClient(INITIAL_SENDER
		);
    	_ref_ir->known = new byte[MAX_KNOWN];
    	_ref_ir->known[0] = _ref_ir->myID;
 _ref_ir->known[1] = _ref_cs->myID;
 _ref_ir->known[2] = _ref_c->myID;

		_ref_ir->constructorInitiateRequester(INITIAL_SENDER
		);
    	_ref_cs->known = new byte[MAX_KNOWN];
    	_ref_cs->known[0] = _ref_cs->myID;
 _ref_cs->known[1] = _ref_ir->myID;
 _ref_cs->known[2] = _ref_ei->myID;

		_ref_cs->constructorCommunicationSubstrate(INITIAL_SENDER
		);
    	_ref_ei->known = new byte[MAX_KNOWN];
    	_ref_ei->known[0] = _ref_ei->myID;
 _ref_ei->known[1] = _ref_cs->myID;
 _ref_ei->known[2] = _ref_s->myID;

		_ref_ei->constructorExecuteInvoker(INITIAL_SENDER
		);
    	_ref_s->known = new byte[MAX_KNOWN];
    	_ref_s->known[0] = _ref_s->myID;
 _ref_s->known[1] = _ref_ei->myID;
