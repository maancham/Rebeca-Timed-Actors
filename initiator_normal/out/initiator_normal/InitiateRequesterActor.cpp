	...
	void InitiateRequesterActor::setNow(TIME_TYPE now) {
...

		{
			...
			if ((((_ref_time-_ref_lastIni)<_ref_N_INI)&(!_ref_theFirsttime))) {
				{
					(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 54"), temp0)->_timed_msg_fastSendFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				{
					(_ref_lastIni=_ref_now);
					(temp1=_ref_cs, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 58"), temp1)->_timed_msg_initiate(myID, (_ref_Lm+_ref_IR1Delay), _ref_IR1Delay + _ref_now, MAX_TIME);
				}
}
...

		{
			...
			if (((_ref_time-_ref_lastIni)>=_ref_L_INI)) {
				{
					(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 68"), temp0)->_timed_msg_timeOutFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				if ((_ref_IR2Delay<_ref_L_INI)) {
					{
						(temp1=_ref_c, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 71"), temp1)->_timed_msg_ack(myID, _ref_Lm, _ref_IR2Delay + _ref_now, MAX_TIME);
					}
}}
		}
		if (__tNumber & 1) {
		...
		{
			(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 75"), temp0)->_timed_msg_excessLoadFailure(myID, _ref_now, MAX_TIME);
		}
		return 0;
		...
		{
			(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 78"), temp0)->_timed_msg_timeOutFailure(myID, _ref_now, MAX_TIME);
		}
		return 0;
