...
void PublishRequesterActor::exportInsideStateVariablesTagInXML(ostream &out, string tab){
			    	...
			    	out << tab << '\t' << "<variable name=\"PublishRequester.lastPub\" type=\"Timer\">" <<  
			    	...
			    	out << tab << '\t' << "<variable name=\"PublishRequester.theFirsttime\" type=\"boolean\">" <<  
...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void PublishRequesterActor::setNow(TIME_TYPE now) {
		...
		{
			(_ref_PR1Delay=(__tNumber|=1, nonDetVariable0));
			int _ref_passedTime = getTimerValue(_ref_lastPub);

			stopTimer(_ref_lastPub);
			if (((_ref_passedTime<_ref_NPUB)&(!_ref_theFirsttime))) {
				{
					(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 58"), temp0)->_timed_msg_fastPublicationFailure(myID, _ref_now, MAX_TIME);
				}
}
			if ((_ref_PR1Delay>_ref_LPUB)) {
				{
					(temp1=_ref_c, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 61"), temp1)->_timed_msg_timeOutFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				{
					startTimer(_ref_lastPub);
					(temp2=_ref_cs, assertion(temp2!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 66"), temp2)->_timed_msg_publish(myID, (_ref_Lm+_ref_PR1Delay), (_ref_life-_ref_PR1Delay), _ref_PR1Delay + _ref_now, MAX_TIME);
					(temp3=_ref_c, assertion(temp3!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 67"), temp3)->_timed_msg_accepted(myID, _ref_PR1Delay + _ref_now, MAX_TIME);
				}
}
