#include "AbstractTimedRebecaAnalyzer.h"
		...
		#define _ref_pr (dynamic_cast<PublishRequesterActor*>(rebecs[1]))
		#define _ref_cs (dynamic_cast<CommunicationSubstrateActor*>(rebecs[2]))
		#define _ref_si (dynamic_cast<SubscribeInvokerActor*>(rebecs[3]))
...
void AbstractTimedRebecaAnalyzer::instantiationPart() {
	...
	byte maxRebecID = 0;
    	ClientActor* _ref_c = new ClientActor(maxRebecID++, "c", 20, 0, *referenceToRebecs 
    											#ifdef DEBUG_LEVEL_2 
    	...
    	PublishRequesterActor* _ref_pr = new PublishRequesterActor(maxRebecID++, "pr", 20, 8, *referenceToRebecs 
    											#ifdef DEBUG_LEVEL_2 
    											, analysisOutputStream 
    											#endif
    											, 209
        ...
        rebecs.push_back(_ref_pr);
        referenceToRebecs->push_back(_ref_pr);
    	CommunicationSubstrateActor* _ref_cs = new CommunicationSubstrateActor(maxRebecID++, "cs", 20, 8, *referenceToRebecs 
    											#ifdef DEBUG_LEVEL_2 
    											, analysisOutputStream 
    											#endif
    											, 200
    	...
    	SubscribeInvokerActor* _ref_si = new SubscribeInvokerActor(maxRebecID++, "si", 20, 8, *referenceToRebecs 
    											#ifdef DEBUG_LEVEL_2 
    											, analysisOutputStream 
    											#endif
    											, 205
        ...
        rebecs.push_back(_ref_si);
        referenceToRebecs->push_back(_ref_si);
 ...
 _ref_c->known[1] = _ref_pr->myID;

		_ref_c->constructorClient(INITIAL_SENDER
		);
    	_ref_pr->known = new byte[MAX_KNOWN];
    	_ref_pr->known[0] = _ref_pr->myID;
 _ref_pr->known[1] = _ref_cs->myID;
 _ref_pr->known[2] = _ref_c->myID;

		_ref_pr->constructorPublishRequester(INITIAL_SENDER
		);
    	_ref_cs->known = new byte[MAX_KNOWN];
    	_ref_cs->known[0] = _ref_cs->myID;
 _ref_cs->known[1] = _ref_si->myID;

		_ref_cs->constructorCommunicationSubstrate(INITIAL_SENDER
		);
    	_ref_si->known = new byte[MAX_KNOWN];
    	_ref_si->known[0] = _ref_si->myID;
 _ref_si->known[1] = _ref_cs->myID;
 _ref_si->known[2] = _ref_s->myID;

		_ref_si->constructorSubscribeInvoker(INITIAL_SENDER
		);
    	_ref_s->known = new byte[MAX_KNOWN];
    	_ref_s->known[0] = _ref_s->myID;
 _ref_s->known[1] = _ref_si->myID;
