...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void PublishRequesterActor::setNow(TIME_TYPE now) {
...

		{
			...
			if ((((_ref_time-_ref_lastPub)<_ref_NPUB)&(!_ref_theFirsttime))) {
				{
					(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 54"), temp0)->_timed_msg_fastPublicationFailure(myID, _ref_now, MAX_TIME);
				}
}
			if ((_ref_PR1Delay>_ref_LPUB)) {
				{
					(temp1=_ref_c, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 57"), temp1)->_timed_msg_timeOutFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				{
					(_ref_lastPub=_ref_now);
					(temp2=_ref_cs, assertion(temp2!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 61"), temp2)->_timed_msg_publish(myID, (_ref_Lm+_ref_PR1Delay), (_ref_life-_ref_PR1Delay), _ref_PR1Delay + _ref_now, MAX_TIME);
					(temp3=_ref_c, assertion(temp3!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 62"), temp3)->_timed_msg_accepted(myID, _ref_PR1Delay + _ref_now, MAX_TIME);
				}
}
