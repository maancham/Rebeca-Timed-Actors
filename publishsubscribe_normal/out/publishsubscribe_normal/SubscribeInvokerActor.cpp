...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void SubscribeInvokerActor::setNow(TIME_TYPE now) {
...

		{
			...
			if ((((_ref_time-_ref_lastPub)<_ref_NSUB)&(!_ref_theFirsttime))) {
				{
					(temp0=_ref_self, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 103"), temp0)->_timed_msg_drop(myID, _ref_Lm, _ref_now, MAX_TIME);
				}
}
			if (((_ref_time-_ref_lastPub)>_ref_XSUB)) {
				{
					(temp1=_ref_s, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 106"), temp1)->_timed_msg_slowPublication(myID, _ref_Lm, _ref_now, MAX_TIME);
				}
}
			if (((_ref_life<_ref_RSUB)||(_ref_Lm>_ref_RPUB))) {
				{
					(temp2=_ref_s, assertion(temp2!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 109"), temp2)->_timed_msg_staleData(myID, _ref_Lm, _ref_now, MAX_TIME);
				}
}
			else {
				{
					(_ref_lastPub=_ref_now);
					(temp3=_ref_s, assertion(temp3!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 113"), temp3)->_timed_msg_publish(myID, (_ref_Lm+_ref_SI1Delay), _ref_SI1Delay + _ref_now, MAX_TIME);
				}
}
...

		{
			...
			if (((_ref_time-_ref_lastPub)>_ref_LSUB)) {
				{
					(temp0=_ref_s, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 123"), temp0)->_timed_msg_slowConsumption(myID, (_ref_Lm+_ref_SI2Delay), _ref_now, MAX_TIME);
				}
}
		}
		if (__tNumber & 1) {
		...
		{
_ref_now += 1;
			(temp0=_ref_self, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 128"), temp0)->_timed_msg_busy(myID, _ref_now, MAX_TIME);
		}
		return 0;
		...
		{
			(temp0=_ref_self, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 129"), temp0)->_timed_msg_busy(myID, _ref_now, MAX_TIME);
		}
		return 0;
