...
long ServiceActor::methodSelectorAndExecutor() {
    ...
    long ServiceActor::constructorService (byte creatorId 
		...
		void ServiceActor::_msg_response (byte senderId
	        ,
    		int
	         _ref_Lm
  	  ){
	...
	vector<TimeFrame>** ServiceActor::getHashtableTimeExtension() {
	    ...
	    void ServiceActor::_timed_msg_response (byte senderId
	        ,
    		int
	         _ref_Lm
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void ServiceActor::setNow(TIME_TYPE now) {
	    ...
	    long ServiceActor::msgsrvresponse (
    		int
	         _ref_Lm
    ) {
...
_ref_now += _ref_computeDelay;
			(temp0=_ref_RI, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 200"), temp0)->_timed_msg_response(myID, (_ref_Lm+_ref_computeDelay), _ref_life, _ref_now, MAX_TIME);
		}
		if (__tNumber & 1) {
