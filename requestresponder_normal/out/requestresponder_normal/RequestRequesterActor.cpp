...
void RequestRequesterActor::exportInsideStateVariablesTagInXML(ostream &out, string tab){
			    	...
			    	out << tab << '\t' << "<variable name=\"RequestRequester.lastReq\" type=\"int\">" <<  
			    		((int)_ref_lastReq) << "</variable>" << endl;
...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void RequestRequesterActor::setNow(TIME_TYPE now) {
...

		{
			...
			if ((((_ref_time-_ref_lastReq)<_ref_NREQ)&(!_ref_theFirsttime))) {
				{
					(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 58"), temp0)->_timed_msg_fastRequestFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				{
					(_ref_lastReq=_ref_now);
					(temp1=_ref_cs, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 62"), temp1)->_timed_msg_request(myID, (_ref_Lm+_ref_RR1Delay), _ref_RR1Delay + _ref_now, MAX_TIME);
				}
}
...

		{
			...
			if (((_ref_life>0)&&(_ref_life<_ref_RREQ))) {
				{
					(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 73"), temp0)->_timed_msg_staleData(myID, _ref_now, MAX_TIME);
					(_ref_canConsume=false);
				}
}
			if (((_ref_time-_ref_lastReq)>=_ref_LREQ)) {
				{
					(temp1=_ref_c, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 77"), temp1)->_timed_msg_timeOutFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				if ((_ref_canConsume&&(_ref_RR2Delay<_ref_LREQ))) {
					{
						(temp2=_ref_c, assertion(temp2!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 80"), temp2)->_timed_msg_response(myID, _ref_Lm, _ref_RR2Delay + _ref_now, MAX_TIME);
					}
}}
		}
		return 0;
		...
		{
			(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 85"), temp0)->_timed_msg_excessLoadFailure(myID, _ref_now, MAX_TIME);
		}
		return 0;
		...
		{
			(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 88"), temp0)->_timed_msg_timeOutFailure(myID, _ref_now, MAX_TIME);
		}
		return 0;
		...
		{
			(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 91"), temp0)->_timed_msg_dataUnavailableFailure(myID, _ref_now, MAX_TIME);
		}
		return 0;
