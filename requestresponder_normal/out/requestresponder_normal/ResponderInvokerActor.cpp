...
void ResponderInvokerActor::exportInsideStateVariablesTagInXML(ostream &out, string tab){
			    	...
			    	out << tab << '\t' << "<variable name=\"ResponderInvoker.lastReq\" type=\"int\">" <<  
...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void ResponderInvokerActor::setNow(TIME_TYPE now) {
...

		{
			...
			if ((((_ref_time-_ref_lastReq)<_ref_NRES)&(!_ref_theFirsttime))) {
				{
					(temp0=_ref_cs, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 147"), temp0)->_timed_msg_excessLoadFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				{
					(_ref_lastReq=_ref_now);
					(temp1=_ref_s, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 152"), temp1)->_timed_msg_response(myID, _ref_Lm, _ref_RI1Delay + _ref_now, MAX_TIME);
				}
}
...

		{
			...
			if ((_ref_life<_ref_RRES)) {
				{
					(temp0=_ref_cs, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 162"), temp0)->_timed_msg_dataUnavailableFailure(myID, _ref_now, MAX_TIME);
				}
}
			if (((_ref_time-_ref_lastReq)>_ref_LRES)) {
				{
					(temp1=_ref_cs, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 165"), temp1)->_timed_msg_timeOutFailure(myID, _ref_now, MAX_TIME);
				}
}
			else {
				{
					(temp2=_ref_cs, assertion(temp2!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 168"), temp2)->_timed_msg_response(myID, (_ref_Lm+_ref_RI2Delay), (_ref_life-_ref_RI2Delay), _ref_RI2Delay + _ref_now, MAX_TIME);
				}
}
		}
		return 0;
