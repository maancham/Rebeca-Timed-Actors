...
void SendRequesterActor::exportInsideStateVariablesTagInXML(ostream &out, string tab){
			    	...
			    	out << tab << '\t' << "<variable name=\"SendRequester.lastSend\" type=\"Timer\">" <<  
...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void SendRequesterActor::setNow(TIME_TYPE now) {
...

		{
			...
			stopTimer(_ref_lastSend);
			if (((_ref_passedTime<_ref_nSen)&(!_ref_theFirsttime))) {
				{
					(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 60"), temp0)->_timed_msg_fastSendFailure(myID, (_ref_Lm+_ref_SR1Delay), _ref_now, MAX_TIME);
				}
}
			else {
				{
					startTimer(_ref_lastSend);
					(temp1=_ref_cs, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 65"), temp1)->_timed_msg_send(myID, (_ref_Lm+_ref_SR1Delay), _ref_SR1Delay + _ref_now, MAX_TIME);
				}
}
...

		{
			...
			stopTimer(_ref_lastSend);
			if ((_ref_passedTime>=_ref_lSen)) {
				{
					(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 77"), temp0)->_timed_msg_timeOutFailure(myID, _ref_Lm, _ref_now, MAX_TIME);
				}
}
			else {
				if ((_ref_SR2Delay<_ref_lSen)) {
					{
						(temp1=_ref_c, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 80"), temp1)->_timed_msg_ack(myID, _ref_Lm, _ref_SR2Delay + _ref_now, MAX_TIME);
					}
}}
		}
		if (__tNumber & 1) {
		...
		{
			(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 86"), temp0)->_timed_msg_excessLoadFailure(myID, _ref_Lm, _ref_now, MAX_TIME);
		}
		return 0;
		...
		{
			(temp0=_ref_c, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 89"), temp0)->_timed_msg_timeOutFailure(myID, _ref_Lm, _ref_now, MAX_TIME);
		}
		return 0;
