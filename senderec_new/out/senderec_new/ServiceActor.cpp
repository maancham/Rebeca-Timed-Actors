...
#define _ref_self (dynamic_cast<ServiceActor*>(rebecs[myID]))
    	#define _ref_RI (dynamic_cast<ReceiveInvokerActor*>(rebecs[known[1]]))
...
long ServiceActor::methodSelectorAndExecutor() {
    ...
    long ServiceActor::constructorService (byte creatorId 
		...
		void ServiceActor::_msg_send (byte senderId
	        ,
    		int
	         _ref_Lm
  	  ){
	...
	vector<TimeFrame>** ServiceActor::getHashtableTimeExtension() {
	    ...
	    void ServiceActor::_timed_msg_send (byte senderId
	        ,
    		int
	         _ref_Lm
...
_ref_now += _ref_computeDelay;
			(temp0=_ref_RI, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 189"), temp0)->_timed_msg_ack(myID, (_ref_Lm+_ref_computeDelay), _ref_now, MAX_TIME);
		}
		if (__tNumber & 1) {
