...
void ReceiveInvokerActor::exportInsideStateVariablesTagInXML(ostream &out, string tab){
			    	...
			    	out << tab << '\t' << "<variable name=\"ReceiveInvoker.lastSend\" type=\"int\">" <<  
...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void ReceiveInvokerActor::setNow(TIME_TYPE now) {
...

		{
			...
			if ((((_ref_time-_ref_lastSend)<_ref_nRec)&(!_ref_theFirsttime))) {
				{
					(temp0=_ref_cs, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 141"), temp0)->_timed_msg_excessLoadFailure(myID, _ref_Lm, _ref_now, MAX_TIME);
				}
}
			else {
				{
					(_ref_lastSend=_ref_now);
					(temp1=_ref_s, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 145"), temp1)->_timed_msg_send(myID, _ref_Lm, _ref_RI1Delay + _ref_now, MAX_TIME);
				}
}
...

		{
			...
			if (((_ref_time-_ref_lastSend)>_ref_lRec)) {
				{
					(temp0=_ref_cs, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 155"), temp0)->_timed_msg_timeOutFailure(myID, (_ref_Lm+_ref_RI2Delay), _ref_now, MAX_TIME);
				}
}
			else {
				{
					(temp1=_ref_cs, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 158"), temp1)->_timed_msg_ack(myID, (_ref_Lm+_ref_RI2Delay), _ref_RI2Delay + _ref_now, MAX_TIME);
				}
}
		}
		if (__tNumber & 1) {
