...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	...
	void CommunicationSubstrateActor::setNow(TIME_TYPE now) {
...

		{
			...
			(temp0=_ref_ei, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 135"), temp0)->_timed_msg_initiate(myID, _ref_action, (_ref_Lm+_ref_csDelay), _ref_csDelay + _ref_now, MAX_TIME);
		}
		if (__tNumber & 1) {
...

		{
			...
			(temp0=_ref_ir, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 140"), temp0)->_timed_msg_ack(myID, _ref_action, _ref_actionStatus, (_ref_Lm+_ref_cs2Delay), _ref_cs2Delay + _ref_now, MAX_TIME);
		}
		if (__tNumber & 1) {
			if (false){/*Dummy 'if' statement to create nested else-if easily!*/}
			else if(nonDetVariable0 == 3) {
...

		{
			...
			(temp0=_ref_ir, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 144"), temp0)->_timed_msg_excessLoadFailure(myID, _ref_action, _ref_Lm, _ref_now, MAX_TIME);
		}
		return 0;
...

		{
			...
			(temp0=_ref_ir, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 148"), temp0)->_timed_msg_timeOutFailure(myID, _ref_action, _ref_actionStatus, _ref_Lm, _ref_now, MAX_TIME);
		}
		return 0;
